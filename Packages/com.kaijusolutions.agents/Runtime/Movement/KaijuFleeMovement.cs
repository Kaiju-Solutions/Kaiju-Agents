...
namespace KaijuSolutions.Agents.Movement
{
    ...
    public class KaijuFleeMovement : KaijuLeavingMovement
    {
        ...
        /// </summary>
        /// <param name="agent">The agent this is assigned to.</param>
        /// <param name="target">The transform to flee from.</param>
        /// <param name="distance">The distance from the target to consider this movement done.</param>
        ...
        {
            Vector2? t = Target;
            return $"Kaiju Flee Movement - Agent: {(Agent ? Agent.name : "None")} - Target: {(t.HasValue ? t.ToString() : "None")} - Distance: {Distance} - Current Distance: {CurrentDistance} - {(Done() ? "Done" : "Executing")}";
        }
    }
}