...
namespace KaijuSolutions.Agents.Movement
{
    ...
    public abstract class KaijuApproachingMovement : KaijuTargetMovement
    {
        ...
        {
            Vector2? t = Target;
            return $"Kaiju Approaching Movement - Agent: {(Agent ? Agent.name : "None")} - Target: {(t.HasValue ? t.Value.ToString() : "None")} - Distance: {Distance} - Current Distance: {CurrentDistance} - Weight: {Weight} - {(Done() ? "Done" : "Executing")}";
        }
    }
}