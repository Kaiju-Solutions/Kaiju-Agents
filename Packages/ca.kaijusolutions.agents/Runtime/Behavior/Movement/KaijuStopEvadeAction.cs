...
namespace KaijuSolutions.Agents.Behavior.Movement
{
    ...
    [NodeDescription(
        ...
        description:
        "Stop all evade movements on an agent. This will report a failure if the agent is not currently evading. This will report a failure if the agent is not currently moving. If the agent is not assigned, it will try to find an agent variable. If none are found, it will try to find an agent attached to any other component and create a variable from it.",
