...
namespace KaijuSolutions.Agents.Behavior.Movement
{
    ...
    public class KaijuSeparationAction : KaijuMovementAction<KaijuSeparationMovement>
    {
        ...
        {
            return $"Kaiju Separation Action - Agent: {(agent.Value ? agent.Value : "None")} - Configuration: {(configuration.Value ? configuration.Value : "None")} - Movement: {(Movement ? Movement : "None")}";
        }
    }
}
#endif