...
{
    /// <summary>
    /// Base <see cref="KaijuSolutions.Agents.Movement.KaijuMovement"/> class for moving away from a single target.
    /// </summary>
    public abstract class KaijuLeavingMovement : KaijuTargetMovement
    {
        /// <summary>
        /// The default  <see cref="KaijuSolutions.Agents.Movement.KaijuTargetMovement.Distance"/> of leaving movements.
        /// </summary>
        ...
        {
            return $"Kaiju Leaving Movement - Agent: {(Agent ? Agent : "None")} - Target: {Target.ToString()} - Distance: {Distance} - Current Distance: {CurrentDistance} - Weight: {Weight} - {(Done() ? "Done" : "Executing")}";
        }
    }
}