...
namespace KaijuSolutions.Agents.Movement
{
    ...
    public class KaijuPursueMovement : KaijuSeekMovement
    {
        ...
        /// <returns>Get a pursue movement for the <see cref="KaijuSolutions.Agents.KaijuAgent"/>.</returns>
        ...
        /// <param name="distance">The distance from the target to consider this movement done.</param>
        /// <param name="weight">The weight of this <see cref="KaijuSolutions.Agents.Movement.KaijuMovement"/>.</param>
        public KaijuPursueMovement([NotNull] KaijuAgent agent, [NotNull] Component target, float distance = DefaultDistance, float weight = DefaultWeight) : base(agent, target, distance, weight) { }
        ...
        {
            Vector2 pursue = position.Pursue(target, Previous, speed, delta, out Vector2 future);
            Future = future;
            Previous = target;
            return pursue;
        }
        ...
        {
            return $"Kaiju Pursue Movement - Agent: {(Agent ? Agent : "None")} - Target: {Target.ToString()} - Distance: {Distance} - Current Distance: {CurrentDistance} - Previous: {Previous} - Weight: {Weight} - {(Done() ? "Done" : "Executing")}";
        }
    }
}